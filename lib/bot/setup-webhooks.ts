...
// /**
//  * ⚠️ IMPORTANT
//  * This file MUST run in a Node.js runtime.
//  * It must NOT be imported into client components or Edge runtime.
//  */
...
// if (!APP_URL) {
//   console.warn("[Webhook Setup] APP_URL is not defined")
// }

// if (!BACKEND_BASE_URL) {
//   console.warn("[Webhook Setup] BACKEND_BASE_URL is not defined")
// }
...
//       { command: "start", description: "🎮 Start Game | ጨዋታ ጀምር" },
...
//   // Your agentsData keys are numbers (1,2,...)
//   const numericId = Number(agentIdFromBackend);
//   if (!Number.isFinite(numericId)) {
//     console.warn(
//       `[Webhook Setup] agent.id="${agentIdFromBackend}" is not numeric. ` +
//       `Cannot read admins from agentsData.`
//     );
//     return [];
//   }
...
//      // 2) Admin commands for specific admin users (THIS is what worked for you)
//     const adminIds = getAdminsForAgentId(id);

//     if (!adminIds.length) {
//       console.warn(
//         `[Webhook Setup] No admin IDs found for agent ${id}. ` +
//         `Broadcast command will not appear for anyone.`
//       );
//     }
...
//     agents.map((agent) => setupAgentWebhook(agent))
//   )
...
// export async function setupSingleAgentWebhook(
//   agentId: string,
//   botToken: string
// ): Promise<boolean> {
...
/**
 * Multi-agent webhook setup + admin command enable-on-/start (Option A)
 *
 * What this does:
 * 1) Sets webhook for each agent bot (token from backend).
 * 2) Sets BASE_COMMANDS globally (everyone sees these).
 * 3) Tries to set ADMIN_COMMANDS per-admin (scope chat). If admin hasn't started the bot yet -> skips.
 * 4) IMPORTANT: When an admin starts the bot (/start), your runtime handlers should call
 *    `enableAdminCommandsOnStart(bot, agentId)` so the admin commands get applied immediately
 *    (no "chat not found").
 *
 * Note:
 * - This file only configures webhooks + commands. It does NOT process updates.
 * - You still need to call enableAdminCommandsOnStart(bot, agentId) inside your bot runtime
 *   (where you register all handlers) for Option A to work.
 */
