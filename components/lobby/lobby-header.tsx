...
import { ChevronDown, CreditCard, Info, ReceiptIcon, Settings, Wallet } from "lucide-react"
...
export function LobbyHeader() {
  ...
  const {activeAgentId, agentDetails, loading: agentLoading} = useAgentStore();
  const user = userStore((state) => state.user);

  // alert("USER DATA IN HEADER: " + JSON.stringify(user));
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center justify-between w-full">
              ...
              <div className="flex items-center space-x-4">
                <Link
                  href={`/${i18n.language}?agentId=${activeAgentId}`}
              ...
              {user && (
                <DropdownMenu>
                  ...
                  >
                    <DropdownMenuItem
                      onClick={() =>
                        router.push(`/${i18n.language}/user/update-password?agentId=${activeAgentId}`)
                      }
                      className="
                        cursor-pointer
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-4">
              ...
              {(user?.role === UserRole.ADMIN || user?.role === UserRole.AGENT) && Number(user.agentId) === Number(activeAgentId) &&
